...
public class TileEntityFluxStorage extends TileEntity implements ITickable
{
    	...
    	if(!this.world.isRemote) {
	    	if(!this.handler.getStackInSlot(0).isEmpty() && isItemFuel(this.handler.getStackInSlot(0)))
			{
				...
				if(this.cookTime == 25)
				{
					...
					if(this.handler.getStackInSlot(0).getItem().getUnlocalizedName().equals(FluidUtil.getFilledBucket(new FluidStack(ModFluids.FLUX_FLUID, 1)).getItem().getUnlocalizedName())) {
	...
	{
		if(stack.getItem() == FluidUtil.getFilledBucket(new FluidStack(ModFluids.FLUX_FLUID, 1)).getItem()) return 200;
		else return 0;
	}
	...
	{
		return new TextComponentTranslation("tile.flux_storage_block.name");
	}
